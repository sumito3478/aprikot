...
class ByteStringReader(
  val data: ByteString, override val offset: Int) extends Reader[Byte] {
  def this(data: ByteString) = this(data, 0)
  ...
  override def atEnd = offset >= data.size

  override def first = data(offset)

  override def pos = new Position(offset)

  override def rest = new ByteStringReader(data, offset + 1)
}